...
namespace MrPathV2.Editor.Windows
{
    ...
    public class SelectTerrainLayerWindow : EditorWindow
    {
        ...
        private void QueryUIElements()
        {
            ...
            _sizeLabel = rootVisualElement.Q<Label>(name: "Szie");
        ...
        private VisualElement MakeTile(TerrainLayer tl)
        {
            ...
            var nameLabel = new Label { name = "LayerName" };
        ...
        private bool AreDetailElementsAvailable()
        {
            return _nameLabel != null && _covLabel != null && _iconBox != null;
        }
        ...
        private void MarkPreviewDirty()
        {
            try
            {
                ...
                method.Invoke(null, new object[] { _contextPathCreator, false, false, true });
            }
            catch (Exception ex)
            {
        ...
        private static bool AreTerrainsAvailable(UnityEngine.Terrain[] terrains)
        {
            return terrains is { Length: > 0 };
        }

        private static void ProcessTerrainLayers(UnityEngine.Terrain[] terrains, List<TerrainLayer> result)
        {
            result.AddRange(from terrain in terrains where
                terrain.terrainData != null
                select terrain.terrainData.terrainLayers into layers
                where layers != null && layers.Length != 0 from layer in layers
                where layer != null select layer);
        }
        ...
        private static bool IsTerrainDataAvailable(UnityEngine.Terrain terrain)
        {
            return terrain.terrainData != null;
        }

        private static bool AreLayersAvailable(TerrainLayer[] layers)
        {
            return layers is { Length: > 0 };
        }
                                           ...
                                           Texture2D targetDiffuse, Texture2D targetNormal)
        {
            return layers.
                ...
                Any(layer => IsEquivalentLayer(layer, targetLayer, targetDiffuse, targetNormal));
        ...
        private static bool IsCreatorValid(PathCreator creator)
        {
            return creator != null && creator.profile != null && 
        ...
        private static bool IsTerrainValid(UnityEngine.Terrain terrain)
        {
        ...
        private static bool IsTerrainAndLayerValid(UnityEngine.Terrain terrain, TerrainLayer layer)
        {
            return terrain != null && terrain.terrainData != null && layer != null;
        }
                                                          ...
                                                          Texture2D targetDiffuse, Texture2D targetNormal)
        {
            return layers.
                Where(l => l != null).
                Any(l => IsEquivalentLayer(l, targetLayer, targetDiffuse, targetNormal));

...
namespace MrPathV2.Runtime.Core.BlendMasks
{
    ...
    public class BrushStrokeNoiseMask : BlendMaskBase
    {
        ...
        public override float Evaluate(float horizontalPosition, float pathProgress, float worldWidth, float pathLength)
        {
            ...
            var p = u * Mathf.Max(0.0001f, scale);

            // 确定当前位置所在的“格子”
            var cellIndex = Mathf.Floor(p);
            ...
            for (var i = -1; i <= 1; i++)
            {
                var currentCell = cellIndex + i;

                // --- 为每个格子生成一个独一无二、但始终不变的笔触 ---
                // 1. 随机化笔触的宽度
                var randomWidth = strokeWidth * (1f + (Hash(currentCell, 10.5f) - 0.5f) * 2f * widthVariation);

                // 2. 随机化笔触的强度
                var randomStrength = strength * (1f + (Hash(currentCell, 20.3f) - 0.5f) * 2f * strengthVariation);

                // 3. 随机化笔触在格子内的位置
                var randomOffset = (Hash(currentCell, 30.1f) - 0.5f) * jitter;
                var strokeCenter = currentCell + 0.5f + randomOffset;

                // --- 计算当前点受这个笔触的影响程度 ---
                // 计算点到笔触中心的距离
                var dist = Mathf.Abs(p - strokeCenter);

                // SmoothStep 形成笔触圆顶
                var halfWidth = randomWidth / 2f;
                if (dist < halfWidth)
                {
                    var strokeValue = SmoothStep(halfWidth, 0, dist);
        ...
        private float Hash(float n, float salt) =>
            ...
            Mathf.Abs(Mathf.Sin(n * 12.9898f + salt * 53.123f) * 43758.5453f % 1.0f);

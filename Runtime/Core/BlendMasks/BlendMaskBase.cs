...
namespace MrPathV2._2.Runtime.Core.BlendMasks
{
    ...
    public abstract class BlendMaskBase : ScriptableObject
    {
        ...
        /// </summary>
        public virtual void FillGpuParams(ref GpuMaskParamsData dst)
        {
        ...
        protected float TransformPosition(float horizontalPosition, float worldWidth)
        {
            ...
            var u01 = (horizontalPosition + 1f) * 0.5f;
            var repeatX = Mathf.Abs(tiling.x) < 1e-4f
        ...
        /// </summary>
        protected float TransformPosition(float horizontalPosition, float worldWidth, float pathLength) => TransformPosition(horizontalPosition, worldWidth);
        ...
        protected float TransformPathPosition(float pathProgress, float pathLength)
        {
            ...
            var repeatY = Mathf.Abs(tiling.y) < 1e-4f ? 1e-4f * Mathf.Sign(tiling.y == 0 ? 1f : tiling.y) : tiling.y;
        ...
        protected float ApplySmoothing(float maskValue)
        {
            ...
            var t = (maskValue - edge0) / (edge1 - edge0); // 0..1
